...
keypoints = sift.detect(grey_resized, None)
...
output_path = script_dir / "sift_keypoints.jpg"
cv2.imwrite(str(output_path), keypoint_img)
print(f"SIFT keypoints image saved to: {output_path}")
output_path_rich = script_dir / "sift_rich_keypoints.jpg"
cv2.imwrite(str(output_path_rich), rich_keypoint_img)
print(f"SIFT rich keypoints image saved to: {output_path_rich}")

# create a numPy array of the keypoint coordinates
sift = cv2.SIFT_create()
keypoints, descriptors = sift.detectAndCompute(grey_resized, None)
